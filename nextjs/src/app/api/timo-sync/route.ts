...
function getHash(item: TimoTxn): string {
...
export async function GET(req: Request) {
  ...
  (async () => {
    try {
      await sendLog("info", "🔄 Bắt đầu tiến trình đồng bộ dữ liệu...");
      await sendLog("info", "📡 Đang kết nối tới ngân hàng Timo và Google Sheets...");
      ...
      await sendLog("info", `Đã tải thành công ${existingHashes.size} mã Hash hiện có.`);
      ...
      await sendLog("success", "✅ Đã lấy dữ liệu thành công. Bắt đầu đối chiếu giao dịch...");
      ...
      if (newTxns.length === 0) {
        await sendLog("info", "💤 Không có giao dịch mới nào kể từ lần đồng bộ trước.");
        await closeStream();
        return;
      }
      ...
      for (const tx of newTxns) {
        await sendLog("success", `✨ Phát hiện giao dịch mới: ${tx.item.txnTitle} (${Math.abs(tx.item.txnAmount)} VND)`);
      ...
      await sendLog("info", "Bắt đầu Batch Update vào Google Sheets...");
      ...
      if (batchIncomes.length > 0) {
        ...
        await sendLog("success", `   ➜ Đã ghi thành công ${batchIncomes.length} dòng Thu Nhập`);
      }
      ...
      await sendLog("success", `🎉 Hoàn tất! Đã đồng bộ thành công ${newTxns.length} giao dịch mới.`);
